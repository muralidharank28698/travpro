...
export default function SettingsPage() {
  ...
  const tabs = [
    { id: "personal" as SettingsTab, label: t('tabs.personal'), icon: User },
    { id: "security" as SettingsTab, label: t('tabs.security'), icon: Lock },
    { id: "notifications" as SettingsTab, label: t('tabs.notifications'), icon: Bell },
    { id: "payments" as SettingsTab, label: t('tabs.payments'), icon: CreditCard },
  ];
    ...
    <div className="w-full animate-fade-in-up">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
        <div className="space-y-1">
          ...
          <p className="text-[var(--muted)] font-medium text-sm">{t('subtitle')}</p>
        </div>
      ...
      <div className="grid lg:grid-cols-4 gap-8">
        ...
        <div className="lg:border-r lg:border-border/40 lg:pr-6">
          <p className="text-[10px] font-black uppercase tracking-[0.25em] text-[var(--muted-light)] mb-4 px-3">{t('general')}</p>
          {tabs.map((tab) => {
                ...
                <form onSubmit={handleSave} className="space-y-8">
                  ...
                  <div className="border-b border-border/50 pb-6 flex items-center gap-6">
                    <div className="w-20 h-20 md:w-24 md:h-24 rounded-[2.5rem] bg-surface flex items-center justify-center relative group cursor-pointer border border-border shadow-xl overflow-hidden shrink-0 shadow-primary/5 group-hover:border-primary transition-all duration-300">
                      ...
                      <div className="absolute inset-0 bg-emerald-900/60 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center">
                        <span className="text-white text-[10px] font-black uppercase tracking-widest">{t('personal.update')}</span>
                      </div>
                    </div>
                    <div className="space-y-1.5 flex-1 min-w-0">
                      <h2 className="text-2xl font-black tracking-tight text-[var(--foreground)]">Murali Dharan</h2>
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="text-[10px] font-bold text-primary bg-primary/10 leading-none py-1.5 px-3 rounded-full border border-primary/20 uppercase tracking-[0.1em]">{t('personal.premium')}</span>
                  ...
                  <div className="grid sm:grid-cols-2 gap-x-6 gap-y-6">
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase tracking-[0.2em] font-black text-[var(--muted-light)] ml-1">{t('personal.full_name')}</label>
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase tracking-[0.25em] font-black text-slate-400 ml-1">{t('personal.email')}</label>
                      <div className="relative group">
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase tracking-[0.25em] font-black text-slate-400 ml-1">{t('personal.phone')}</label>
                      <div className="relative group">
              ...
              <div className="mt-6 card p-6 md:p-8 bg-card border-red-500/20 flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden ring-1 ring-red-500/10 shadow-sm transition-all hover:bg-red-500/5 group/danger">
                ...
                <div className="space-y-2 relative z-10">
                  <div className="flex items-center gap-3 text-red-600">
                    ...
                    <h3 className="text-sm font-black tracking-[0.2em] uppercase">{t('personal.danger_title')}</h3>
                  </div>
                  <p className="text-xs text-[var(--muted)] max-w-sm font-semibold opacity-70">
                    {t('personal.danger_desc')}
                  </p>
                </div>
            ...
            <div className="space-y-6 animate-fade-in-up">
              ...
              <div className="card-elevated p-6 md:p-8 space-y-6 shadow-[0_20px_50px_rgba(0,0,0,0.05)] border-emerald-50 bg-card/70 backdrop-blur-md relative overflow-hidden ring-1 ring-emerald-50/50">
                ...
                <div className="border-b border-border/50 pb-6 flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-xl font-black tracking-tight text-[var(--foreground)]">{t('security.change_pw')}</h2>
                    <p className="text-xs text-[var(--muted)] font-medium">{t('security.change_pw_desc')}</p>
                  </div>
                </div>

                <form onSubmit={handleSave} className="space-y-6">
                  <div className="space-y-2">
                    <label className="text-[10px] uppercase tracking-[0.2em] font-black text-[var(--muted-light)] ml-1">{t('security.current_pw')}</label>
                    <div className="relative group">
                  ...
                  <div className="grid sm:grid-cols-2 gap-x-6 gap-y-6">
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase tracking-[0.2em] font-black text-[var(--muted-light)] ml-1">{t('security.new_pw')}</label>
                      <div className="relative group">
                        <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-[18px] h-[18px] text-slate-400" />
                        <input
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase tracking-[0.2em] font-black text-[var(--muted-light)] ml-1">{t('security.confirm_pw')}</label>
                      <div className="relative group">
                        <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-[18px] h-[18px] text-slate-400" />
                        <input
                          type="password"
                  ...
                  <div className="space-y-2">
                    <p className="text-[10px] uppercase tracking-[0.2em] font-black text-[var(--muted-light)] ml-1">{t('security.pw_strength')}</p>
                    ...
                    <p className="text-[11px] text-emerald-600 font-semibold">{t('security.pw_hint')}</p>
                  </div>
              ...
              <div className="card p-6 md:p-8 space-y-6 shadow-xl border border-border/60 bg-card/50 backdrop-blur-md relative overflow-hidden transition-all duration-300">
                <div className="flex items-center gap-4 pb-6 border-b border-border/20">
                  ...
                  <div className="flex-1">
                    <h2 className="text-lg font-black tracking-tight text-[var(--foreground)]">{t('security.twofa_title')}</h2>
                    <p className="text-xs text-[var(--muted)] font-medium">{t('security.twofa_desc')}</p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between p-5 rounded-2xl bg-surface/50 border border-border/60 transition-all hover:border-border/80">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-sm font-bold text-[var(--foreground)]">{t('security.auth_app')}</p>
                        <p className="text-xs text-[var(--muted)]">{t('security.auth_app_desc')}</p>
                      </div>
                    </div>
                    <button
            ...
            <div className="space-y-6 animate-fade-in-up">
              <div className="card-elevated p-6 md:p-8 space-y-6 shadow-[0_20px_50px_rgba(0,0,0,0.05)] bg-card/70 backdrop-blur-md ring-1 ring-emerald-50/50">
                <div className="border-b border-border/50 pb-6 flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-lg font-black tracking-tight text-[var(--foreground)]">{t('notifications.title')}</h2>
                    <p className="text-xs text-[var(--muted)] font-medium">{t('notifications.subtitle')}</p>
                  </div>
                </div>

                {/* Notification Categories */}
                <div className="space-y-3">
                  <h3 className="text-[10px] font-black uppercase tracking-[0.25em] text-[var(--muted-light)] mb-6">{t('notifications.categories')}</h3>
                  ...
                  <div className="flex items-center justify-between p-5 rounded-2xl bg-surface/30 border border-border/40 transition-all hover:border-border/60 group/row">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-sm font-bold text-[var(--foreground)]">{t('notifications.booking_updates')}</p>
                        <p className="text-xs text-[var(--muted)]">{t('notifications.booking_updates_desc')}</p>
                      </div>
                    </div>
                    <button
            ...
            <div className="space-y-6 animate-fade-in-up">
              ...
              <div className="card p-6 md:p-8 space-y-6 shadow-xl border border-border/60 bg-card/50 backdrop-blur-md relative overflow-hidden transition-all duration-300">
                <div className="flex items-center gap-4 pb-6 border-b border-border/20">
                  ...
                  <div>
                    <h2 className="text-lg font-black tracking-tight text-[var(--foreground)]">{t('payments.title')}</h2>
                    <p className="text-xs text-[var(--muted)] font-medium">{t('payments.subtitle')}</p>
                  </div>
                </div>
                ...
                <div className="pt-6 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-border/20">
                  <p className="text-xs text-[var(--muted)] font-medium">
                    {t('payments.showing')} <span className="font-bold text-[var(--foreground)]">{((billingPage - 1) * ITEMS_PER_PAGE) + 1}</span> {t('payments.to')} <span className="font-bold text-[var(--foreground)]">{Math.min(billingPage * ITEMS_PER_PAGE, allInvoices.length)}</span> {t('payments.of')} <span className="font-bold text-[var(--foreground)]">{allInvoices.length}</span> {t('payments.entries')}
                  </p>
