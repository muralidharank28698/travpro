...
export default function BookingsPage() {
  ...
  const statCards = [
    { label: t('total'), value: stats.total, color: "var(--foreground)", filter: null },
    { label: t('confirmed'), value: stats.confirmed, color: "#059669", filter: "Confirmed" as const },
    { label: t('in_progress'), value: stats.inProgress, color: "#2563EB", filter: "In Progress" as const },
    { label: t('completed'), value: stats.completed, color: "var(--muted)", filter: "Completed" as const },
  ];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "32px" }}>
      ...
      <div className="card !p-0 overflow-hidden border border-border/60 bg-card/40 backdrop-blur-md shadow-2xl">
        ...
        <div className="hidden lg:grid grid-cols-[1fr_1.2fr_1fr_0.8fr_0.8fr_0.7fr] gap-4 px-6 py-4 bg-surface/50 border-b border-border/40 text-[10px] font-black uppercase tracking-[0.2em] text-muted">
          <span>{t('headers.booking')}</span>
          <span>{t('headers.route')}</span>
          <span>{t('headers.customer')}</span>
          <span>{t('headers.date')}</span>
          <span>{t('headers.amount')}</span>
        ...
        <div className="divide-y divide-border/20">
          {filteredBookings.length === 0 ? (
            <div className="p-12 flex flex-col items-center justify-center text-center">
              ...
              <h3 className="text-sm font-black text-foreground">{t('empty_title')}</h3>
              <p className="text-xs text-muted mt-2">{t('empty_subtitle')}</p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="flex flex-col">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <span className="text-[10px] font-black text-muted-light px-2.5 py-0.5 border border-border/40 rounded shadow-sm bg-surface/30">
                        {booking.tripType === "Round Trip" ? t('round_trip') : t('one_way')}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 lg:contents gap-4">
                    ...
                    <div className="flex flex-col lg:justify-center">
                      <p className="text-[10px] font-bold lg:hidden text-[var(--muted-light)] uppercase tracking-wider mb-1">{t('headers.customer')}</p>
                    ...
                    {/* Date */}
                    <div className="flex flex-col lg:justify-center">
                      <p className="text-[10px] font-bold lg:hidden text-[var(--muted-light)] uppercase tracking-wider mb-1">{t('headers.schedule')}</p>
                      <p className="text-xs font-semibold text-[var(--foreground)]">{booking.startDate}</p>
                      {booking.startDate !== booking.endDate && (
                        <p className="text-[11px] text-[var(--muted-light)]">{t('to')} {booking.endDate}</p>
                      )}
                    </div>

                    {/* Amount */}
                    <div className="flex flex-col lg:justify-center pt-3 lg:pt-0 border-t border-border/20 lg:border-none">
                      <p className="text-[10px] font-black lg:hidden text-muted-light uppercase tracking-[0.2em] mb-1">{t('headers.fare')}</p>
                    ...
                    <div className="flex flex-col lg:justify-center items-end lg:items-start pt-3 lg:pt-0 border-t border-border/20 lg:border-none">
                      <p className="text-[10px] font-black lg:hidden text-muted-light uppercase tracking-[0.2em] mb-1">{t('headers.status')}</p>
                      <span className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-[10px] font-black border border-border/10 shadow-sm" style={{ backgroundColor: config.badgeBg, color: config.badgeColor }}>
                        ...
                        {t(`status.${booking.status.toLowerCase().replace(' ', '_')}`)}
                      </span>
                    </div>
                  </div>
                </div>
